...
このルールは `Tests/test_ga4_host_guard.py` が検査する。新しいページを追加したら実行する:

```
python3 Tests/test_ga4_host_guard.py
```

gtag.js を読み込むページにガードが無い / ガードが読み込みタグより後ろにある / 測定 ID や
本番ホスト（CNAME）と食い違う場合に落ちる。ガードを持たないページをローカル配信すると、
その page_view や CTA クリックがそのまま本番プロパティに入る。
